...
import type { SetPrescription, Template, TemplateType, WeekPlan } from "../types";
			...
			if (token.depth === 1) {
				...
				if (currentSection === "main" || currentSection === "supplemental") {
					flushWeekPlan(
						currentWeeks,
						currentSets,
						currentSection === "main" ? mainWork : supplemental,
					);
					currentWeeks = [];
					currentSets = [];
				}
				...
				token.depth === 3 &&
				(currentSection === "main" || currentSection === "supplemental")
			) {
				flushWeekPlan(
					currentWeeks,
					currentSets,

import { getAllLifts, getPRs, getProgramState, getSchedule, getWorkoutHistory } from "./db/queries";
...
if (prs.length > 0) {
	...
	for (const pr of prs) {
		console.log(
			`  ${pr.lift.padEnd(9)} ${pr.weight} x ${pr.best_reps} (e1RM ${pr.estimated_1rm}) — ${pr.date}`,
		);
	}
}
...
if (recentWorkouts.length > 0) {
	...
	for (const w of recentWorkouts) {
		...
		console.log(
			`  ${w.date}  ${w.lift.padEnd(9)} wk${w.week} ${w.phase} (${w.template})${amrap}${skipped}`,
		);
	}
}
